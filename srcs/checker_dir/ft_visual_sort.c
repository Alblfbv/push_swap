...
/*   Updated: 2019/03/20 14:34:56 by allefebv         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_process_instruct(t_fptr instruct, t_stacks stacks, t_list *inst)
{
	int	i;
...
int			ft_process_visual_sort(t_stacks stacks, t_rect *rects, t_visu *visu)
{
	...
	while (inst)
	{
		...
		if (visu->size_list <= 250)
			SDL_Delay(4500 / visu->size_list);
		ft_display(stacks, rects, visu, &rev_tab);
		ft_process_instruct(instruct, stacks, inst);
		inst = inst->next;
